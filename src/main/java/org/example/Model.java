...
public class Model {
    public static Result trainLoop(int numRound) throws IOException {
        ...
        for (File file : Objects.requireNonNull(inputDir.listFiles())) {
            ...
            Files.writeString(Paths.get(Utils.MODEL_SAVE_PATH + "/" + file.getName() + ".threshold"), String.valueOf(bestThreshold)); //todo: simplify?
    ...
    public static Map<Double, Double> inference(XGBoost bst, double bestThreshold, double[][] testFeatures, double[] testLabels, String type)
        ...
        for (int i = 0; i < prediction.length; i++) {
            ...
            result.put(prediction[i], predLabels[i]);
        }
